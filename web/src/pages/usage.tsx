...
import type {
  UsageLog, UsageSummaryResponse, UsageQuery, Provider,
} from "@/api/types"
import {
...
export default function UsagePage() {
  ...
  async function load() {
    ...
    if (statusFilter === "error") query.has_error = true
    if (statusFilter === "success") query.has_error = false

    const [sum, res, provRes] = await Promise.all([
      api.usage.getSummary(),
      api.usage.list(query),
      providers.length === 0 ? api.providers.list({ limit: 100 }) : Promise.resolve(null),
    ])
    setSummary(sum)
    setLogs(res.items)
    setTotalLogs(res.page.total)
    if (provRes) setProviders(provRes.items)
    setLoading(false)
  }

  useEffect(() => { load() }, [api, modelFilter, statusFilter])
  ...
  async function openDetail(id: string) {
    ...
    setDetailOpen(true)
    const detail = await api.usage.get(id)
    setDetailLog(detail)
  ...
  const errorRate = summary?.summary && summary.summary.total_requests > 0
    ? ((summary.summary.error_count / summary.summary.total_requests) * 100).toFixed(1)
    : "0"

  const hasFilters = modelFilter || statusFilter !== "all"
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant="outline" className="font-mono text-[10px]">
                      {providerMap[log.provider_id]?.name ?? log.provider_id}
                    </Badge>
                  </TableCell>
          ...
          {detailLoading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <DetailField label="Provider" value={providerMap[detailLog.provider_id]?.name ?? detailLog.provider_id} />
              ...
              {detailLog.has_error && detailLog.error_message && (
                <div className="border border-destructive/30 bg-destructive/5 rounded-md p-3">
                  <p className="font-mono text-xs text-destructive mb-1">Error</p>
              ...
              <div className="border rounded-md p-3">
                ...
                <div className="grid grid-cols-3 gap-3 text-sm">
                  ...
                  <DetailField label="Cached" value={detailLog.cached_tokens.toLocaleString()} />
