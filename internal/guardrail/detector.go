...
var secretsDetectors = []Detector{
	...
	},
	{
		...
		Validate: func(match string, ctx string) bool {
			return !isPlaceholder(match) && shannonEntropy(match) >= 3.0
		},
		Replacement: "[SECRET_REDACTED]",
	},
}
...
func CheckPII(text string) []DetectorMatch { return RunDetectors(text, piiDetectors) }

// CheckSecrets runs secrets detectors on text and returns matches.
func CheckSecrets(text string) []DetectorMatch { return RunDetectors(text, secretsDetectors) }
...
// Returns the first matched pattern or "".
...
func isPlaceholder(s string) bool {
	...
	return allPlaceholder && len(words) > 0
}

var reDoSPat = regexp.MustCompile(`\([^)]*[+*][^)]*\)[+*]`)

func hasReDoSRisk(pattern string) bool { return reDoSPat.MatchString(pattern) }
